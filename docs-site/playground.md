...
This small runner evaluates one program with MeTTa TS. Open the [browser IDE](/browser-ide) when you want live language-server analysis or a multi-file workspace. Atoms without a leading `!` are added to the space; atoms with `!` are evaluated and their results shown.
