...
It is pure SVG with no runtime dependency. GIF export takes an encoder as an argument, so you only install [`gifenc`](https://www.npmjs.com/package/gifenc) if you want that one feature.
