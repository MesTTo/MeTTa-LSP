...
function runSource(src: string, fuel?: number, imports?: Map<string, Atom[]>, opts?: RunOptions): QueryResult[]
function runSourceAsync(
  src: string,
...
function makeBrowserParEvalImpl(
  fuel: number,
  options?: BrowserParEvalOptions,
): (rulesSrc: string, branchSrcs: string[], firstOnly: boolean) => Promise<(string[] | null)[]>
```

`run` evaluates a program in the browser, resolving `import!` against an in-memory virtual file system (`files` maps a module name to its MeTTa source). `runSource` and `runSourceAsync` mirror the Node source runner without filesystem capability. Async forms such as `par`, `race`, and `with-mutex` use the core async evaluator; `(once (hyperpose ...))` can use module Web Workers through `makeBrowserParEvalImpl`. `vfsImports` builds the import map directly.

The whole interpreter is pure TypeScript, so it runs in any browser with no native addon and no WASM, which is exactly what powers the [playground](/playground).
